...
public extension CacheConfiguration {
    ...
    static var production: CacheConfiguration {
        let endpointConfigs: [String: EndpointCacheConfiguration] = [
            ...
            "/api/v3/lego/colors/": EndpointCacheConfiguration(
                expiration: .after(150 * 24 * 60 * 60)), // 150 days
            ...
            // More dynamic data
            "/api/v3/lego/sets/": EndpointCacheConfiguration(expiration: .after(7 * 24 * 60 * 60)), // 1 week
            "/api/v3/lego/minifigs/": EndpointCacheConfiguration(
                expiration: .after(7 * 24 * 60 * 60)), // 1 week
        ]

        return CacheConfiguration(
            defaultExpiration: .after(1 * 24 * 60 * 60), // 1 day default
