...
final class DatabaseCacheTests {
    ...
    @Test func testDatabaseCacheExpiration() async throws {
        ...
        try await cache.store(entity: color, expiration: .after(1))  // 1s
    ...
    @Test func testDatabaseCacheExpirationAtSpecificDate() async throws {
        ...
        // Store with expiration at specific date (1s from now)
    ...
    @Test func testDatabaseCacheConfigurationExpiration() async throws {
        ...
        let config = DatabaseCacheConfiguration(
            defaultExpiration: .after(3),  // 3s default
            entityConfigurations: [
                "colors": EntityDatabaseConfiguration(expiration: .after(1))  // 1s for colors
            ]
        )
        ...
        let colorImmediate = try await cache.getEntity(
            type: Color.self, primaryKey: "12", configuration: config)
        let themeImmediate = try await cache.getEntity(
            type: Theme.self, primaryKey: "1", configuration: config)
        ...
        let colorAfter150 = try await cache.getEntity(
            type: Color.self, primaryKey: "12", configuration: config)
        let themeAfter150 = try await cache.getEntity(
            type: Theme.self, primaryKey: "1", configuration: config)
        ...
        try await Task.sleep(for: .seconds(2))  // Additional 100ms

        let themeAfter250 = try await cache.getEntity(
            type: Theme.self, primaryKey: "1", configuration: config)
    ...
    @Test func testDatabaseCacheExpirationConcurrentAccess() async throws {
        ...
        async let store2: Void = cache.store(entity: color2, expiration: .after(1))  // 300ms
        async let store3: Void = cache.store(entity: color3, expiration: .after(2))  // 300ms
        ...
        let (redAfter, blueAfter, greenAfter) = try await (
            expiredRetrieve1, expiredRetrieve2, expiredRetrieve3
        )
    ...
    @Test func testEnableDatabaseCachingOnSharedConfig() {
        ...
        #expect(
            RebrickableLegoAPIClientAPIConfiguration.shared.requestBuilderFactory
                is DatabaseAwareRequestBuilderFactory)
